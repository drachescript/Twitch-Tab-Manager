...
(() => {
  ...
  T.redactForDiag ??= function redactForDiag(input) {
  ...
  T.diagnose ??= async () => {
    ...
    const openTabs = await T.listOpenTabs();
    const managedOpen = openTabs.filter((t) => T.isManaged(t));
